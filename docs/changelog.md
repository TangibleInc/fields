...
- Dynamic values: a consumer API for products with their own UI (a page builder's dialog, a block editor's toolbar). `window.tangibleFields` now exports `DynamicFieldSettings`, `DynamicValueSettings` (a value's settings sub-form on its own) and a `dynamicValues` namespace (`createAPI`, `buildGroupedChoices`, `stringify`, `parse`, `regex`, `defaultLabels`). `dynamicValues.createAPI({ types, categories, mode })` drives the pickers without a host field.
- Dynamic values: `DynamicFieldSettings` gains `modes` (`['builtin']` hides the Field Type toggle), `extraGroups` (consumer-provided groups listed after the registry, keys submitted unchanged), `labels` (every string; the dialog was hardcoded English), `container` (which also receives the interface wrapper classes while open, so styles and portal roots resolve) and `context`; `onSubmit` receives a second argument describing the pick (`value`, `label`, `group`, `settings`, `extra`). Blank settings no longer travel in the token (`[[user_meta::meta_name=x]]`, not `[[user_meta::source=::meta_name=x]]`). Existing usage is unchanged.
