...
- Dynamic values: `DynamicFieldSettings` gains `modes` (`['builtin']` hides the Field Type toggle), `labels` (every string; the dialog was hardcoded English), `portalContainer` (the same one `renderField()` accepts: the dialog's interface wrapper is created inside it) and `context`; `onSubmit` receives a second argument describing the pick (`value`, `label`, `group`, `settings`). Blank settings no longer travel in the token (`[[user_meta::meta_name=x]]`, not `[[user_meta::source=::meta_name=x]]`). Existing usage is unchanged.
